...
Let's understand the above `sortingDataAccessor` function. 

The function checks if the column name is `project.name`, if it's true then returns `row.project.name`. 
