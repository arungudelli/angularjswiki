...
subtitle = "Complete list of 2016+ free font awesome icons"
type="docs"
...
Depending upon the icon style, these free font awesome icons divided into three categories. 

1. font awesome solid icons
2. font awesome regular icons
3. font awesome brand icons

We can <a href="https://use.fontawesome.com/releases/v6.2.0/fontawesome-free-6.2.0-web.zip" rel="noopener" target="_blank">download</a> font awesome icons css file and serve them from your own server Or we can serve them from public CDNs.
...
3. Additionally we have to add Style class. i.e., solid or regular or brand
...
To use official font awesome CDN. Go through the [font awesome CDN](/fontawesome/cdn) article.

Now to display `facebook` font awesome icon, use the below html code.
...
```
`bookmark` icon is as part of regular icons, add the class name as `far`
...
For example to display `user` icon, We might have used below HTML code.
...
But our official font awesome user icon name is `fa-user`.

So to display the icon, for the `.user` class add CSS content code property as `\f007`, which is unicode value for `user` icon.
...
Along with CSS content code, We need to add `font` style as well.

We can use font awesome predefined CSS custom properties for that.

For solid icons use `var(--fa-font-solid)`

For regular icons use `var(--fa-font-regular)`

For Brand icons use `var(--fa-font-regular)`

In addition to that we can add a custom class which defines few common CSS properties.
...
In font awesome 5 version there are no custom css properties. 

So we need to add `font-family` and `font-weight` css properties.
...
Here is the complete list of free font awesome icons cheat sheet.

Type the icon name in the below search box and copy the class name and style or CSS content code.

The icons will be filtered in the below table.
