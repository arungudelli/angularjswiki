+++
title = "Angular HTTP GET request example with parameters"
subtitle = "Learn how make http get request with parameters in Angular using HttpClient service"
...
1. Create a `HttpParams()` object.
2. Append the parameters to the query parameters object using HttpParams().append() method. 
3. Re-assign the object back to the query parameters object.
4. Pass the query parameter to the second argument of `HttpClient.get()` method using `params` property.
...
Because the `append()` method merges the Http parameters and returns the new instance of the merged HttpParams object on each call to append.
